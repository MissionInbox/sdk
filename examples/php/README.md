...
Single-file walk-through that exercises every method in [`missioninbox/sdk`](https://packagist.org/packages/missioninbox/sdk) against a live environment. Doubles as a tutorial and as an integration test.

**See also:** [package docs](../../packages/php/README.md) for the SDK's full method reference, and the [top-level examples README](../README.md) for the shared design.

## Prerequisites

- PHP 8.1 or newer and Composer
...
- **`ConflictException` when creating a test project or identifier** — a resource with the timestamp-suffixed name already exists (e.g. from an interrupted run). The name embeds a millisecond timestamp, so a fresh run gets past it.
